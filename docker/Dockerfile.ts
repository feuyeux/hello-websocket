...
COPY --from=build-base --chown=node:node /app/hello-websocket-ts/server /app/server
COPY --from=build-base --chown=node:node /app/hello-websocket-ts/client /app/client
COPY --from=build-base --chown=node:node /app/hello-websocket-ts/common /app/common
COPY --from=build-base --chown=node:node /app/hello-websocket-ts/tsconfig.json /app/
USER node
