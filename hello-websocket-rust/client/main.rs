...
async fn main() {
    ...
    log(
        "ws-client",
        "Starting Rust WebSocket client [version: 1.0.0]",
    );
    log(
        "ws-client",
        &format!("Connecting to ws://{}:{}", host, port),
    );

    let path = std::env::var("WS_PATH").unwrap_or_else(|_| "/ws".to_string());
...
async fn try_connect(url: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let hello = Message::Hello {
        client_language: CLIENT_LANG.to_string(),
    };
    ws_sender
        .send(WsMessage::Binary(hello.encode().into()))
        .await?;
    ...
    let random_task = tokio::spawn(async move {
        ...
        loop {
            ...
            if s.send(WsMessage::Binary(rn.encode().into())).await.is_err() {
                break;
            }
            log(
                "ws-client",
                &format!("RANDOM_NUMBER id={} number={}", id, num),
            );
            id += 1;
        }
    });
    ...
    while let Some(msg_result) = ws_receiver.next().await {
        ...
        let decoded = match Message::decode(&data) {
            Ok(m) => m,
            Err(e) => {
                log("ws-client", &format!("Decode error: {}", e));
                continue;
            }
        };
        ...
        match decoded {
            Message::Bonjour { server_language } => {
                log(
                    "ws-client",
                    &format!("BONJOUR server_language={}", server_language),
                );
            }
            ...
            Message::TimeNotification {
                ...
                log(
                    "ws-client",
                    &format!("TIME_NOTIFICATION ts={} iso={}", timestamp_ms, iso8601),
                );
            }
            Message::KissRequest {
                os_name,
                ...
                log(
                    "ws-client",
                    &format!(
                        "KISS_REQUEST os={} ver={} rel={} arch={}",
                        os_name, os_version, os_release, os_architecture
                    ),
                );
            ...
            Message::EchoResponse { status, results } => {
                log(
                    "ws-client",
                    &format!("ECHO_RESPONSE status={} results={}", status, results.len()),
                );
            ...
            Message::HashResponse { id, hash_hex } => {
                log(
                    "ws-client",
                    &format!("HASH_RESPONSE id={} hash={}", id, hash_hex),
                );
            }
