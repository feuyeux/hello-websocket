...
async fn main() {
    ...
    log(
        "ws-server",
        &format!("Starting Rust WebSocket server on port {}", port),
    );
...
async fn handle_connection(stream: tokio::net::TcpStream, _addr: SocketAddr) {
    ...
    let ws_stream = match accept_async_with_config(stream, Some(config)).await {
        Ok(ws) => ws,
        Err(e) => {
            log("ws-server", &format!("Accept error: {}", e));
            return;
        }
    };

    let user_id = format!("rust-{}", &Uuid::new_v4().to_string()[..8]);
    ...
    let ping_task = tokio::spawn(async move {
        loop {
            ...
            if tx_ping.send(ping.encode()).await.is_err() {
                break;
            }
        }
    });
    ...
    let time_task = tokio::spawn(async move {
        loop {
            ...
            let tn = Message::TimeNotification {
                timestamp_ms: now_ms(),
                iso8601: now_iso(),
            };
            if tx_time.send(tn.encode()).await.is_err() {
                break;
            }
        }
    });
    ...
    let kiss_task = tokio::spawn(async move {
        loop {
            ...
            if tx_kiss.send(kr.encode()).await.is_err() {
                break;
            }
        }
    });
            ...
            match tokio::time::timeout(Duration::from_secs(5), ws_receiver.next()).await {
                ...
                Err(_) => {
                    ...
                    if now_ms() - last > SESSION_TIMEOUT_MS as i64 {
                        log("ws-server", &format!("[{}] session timeout", user_id));
                        break;
                    }
                    continue;
                }
        ...
        match decoded {
            Message::Hello {
                ...
                log(
                    "ws-server",
                    &format!(
                        "HELLO from {}, session={}, time={}",
                        cl,
                        session_id,
                        now_ms()
                    ),
                );
                let bonjour = Message::Bonjour {
                    server_language: SERVER_LANG.to_string(),
                };
            ...
            Message::EchoRequest { id, meta, data } => {
                log(
                    "ws-server",
                    &format!("ECHO_REQUEST id={} meta={} data={}", id, meta, data),
                );
            ...
            Message::KissResponse {
                ...
                log(
                    "ws-server",
                    &format!(
                        "KISS_RESPONSE lang={} enc={} tz={}",
                        language, encoding, time_zone
                    ),
                );
            }
            Message::Pong { timestamp_ms } => {
                *last_pong.lock().unwrap() = now_ms();
                log("ws-server", &format!("PONG ts={}", timestamp_ms));
            }
            Message::RandomNumber { id, number } => {
                log(
                    "ws-server",
                    &format!("RANDOM_NUMBER id={} number={}", id, number),
                );
                let hash = hash_number(number);
                let resp = Message::HashResponse {
                    id,
                    hash_hex: hash.clone(),
                };
                let _ = tx.send(resp.encode()).await;
                log(
                    "ws-server",
                    &format!("HASH_RESPONSE id={} hash={}", id, hash),
                );
            }
